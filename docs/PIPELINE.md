...
Files are written to `extras/results/` by default (matches the layout the
friend's scaffold introduced). If the organizers' `eval_metrics.py` expects
a different column layout we adjust the writer in
